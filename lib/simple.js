...
let conn = (opts['legacy'] ? makeWALegacySocket : makeWASocket)(connectionOptions)
  ...
  conn.loadingMsg = async (jid, loamsg, loamsgEdit, loadingMessages, quoted) => {
                const {
                    key
  ...
  */
  ...
  /*
  * Message Modify 
  * Decrypted By @YuuraHz
  */
  
 conn.sendMessageModify = async(jid, teks = '', quoted = '', opts = {}) => {
     return conn.sendMessage(jid, { text: teks, 
         contextInfo: { mentionedJid: conn.parseMention(teks), 
            externalAdReply: { showAdAttribution: opts.ads,
              title: opts.title ?? set.wm, 
               body: opts.body, 
                  mediaType: 1, 
                  previewType: 'PHOTO', 
 ...
 */  
 conn.sendCard = async (jid, title, image, btn = [], options = {}) => {
                    ...
                    viewOnceMessage: {
                        message: {
                            ...
                            interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                                footer: proto.Message.InteractiveMessage.Footer.create({ text: "Powered By YuuraHz" }),
 ...
 * @param title(button text)
 */ 
conn.sendButtonUrl = async (jid, text, title, footer, url, quoted) => {
...
return conn.relayMessage(msgs.key.remoteJid, msgs.message, {
 ...
 * @param title(button text)
 */
conn.sendButtonCopy = async (jid, text, title, footer, kode, quoted) => {
 let msgs = generateWAMessageFromContent(jid, {
  viewOnceMessage: {
    message: {
        messageContextInfo: {
 ...
 * @param title(button text)
 */
 conn.sendQuick = async (jid, array,quoted, json) => {
 const result = [];
  ...
  */
  ...
  conn.getName = async (jid = "", withoutContact = false) => {
    ...
    if (jid.endsWith("@g.us")) return new Promise(async (resolve) => {
      ...
      jid,
      vname: "WhatsApp",
    } : areJidsSameUser(jid, conn.user.id) ? conn.user : {}
    return (
...
exports.smsg = (conn, m, hasParent) => {
  ...
  try {
    if (options && pesan) {
   return await conn.sendFile(m.chat, options, null, pesan, m)
  } else {
   if (pesan) {
    return await conn.reply(m.chat, pesan, m)
   } else {
    return await conn.reply(m.chat, options, m)
   }
  }
 } catch (e) {
  return await conn.reply(m.chat, pesan, m)
  ...
  m.edit = (text, key) => conn.sendMessage(m.chat, { text, edit: key }, { quoted: m })
